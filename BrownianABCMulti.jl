...
@everywhere function softmax(xs::AbstractArray; dims=1)
    max_ = maximum(xs, dims=dims)
    exp_ = exp.(xs .- max_)
    exp_ ./ sum(exp_, dims=dims)
end

@everywhere function my_distance(xs::Array{Float64}, ys::Array{Float64})::Float64
    distance = 0.0
    @inbounds for i in 1:size(xs,1)
        if xs[i] != ys[i] && xs[i] != 0
            distance += 1
        end
    end
